...
/*
Purpose: Initializing hunter
In:
van: The van the hunters will start in first
evList: Evidence list that all hunters share from house
ev: The evidence type that the hunter will collect
name: Name of hunter given by user

Out:
hunter: Pointer of hunter

Output: Initalizes hunter struct in hunter param
*/
...
/*
Purpose: Starts the thread of the hunter when used
In/Out: void* hunterly - void pointer argument containing hunter structure

Output: Simulates and logs all hunters behaviors
*/
void *startHunter(void* hunterly){ //Give them a player number somehow
    ...
    //remove hunter from room when exiting
    for (int i = 0; i < NUM_HUNTERS; i++){
        if ((*hunter)->curRoom->hunters[i] == (*hunter)){
            (*hunter)->curRoom->hunters[i] = NULL;
        }
